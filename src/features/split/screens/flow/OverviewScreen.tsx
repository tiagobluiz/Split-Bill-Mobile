...
import { isItemAssigned, isVisibleItem } from "../shared/recordUtils";
    ...
    >
      ...
      >
        <YStack gap="$5">
          ...
          {settlement?.ok ? (
            <>
              <HeroCard eyebrow="Bill total" title={formatMoney(settlement.data.totalCents, settlement.data.currency, locale)} subtitle={`${record.values.participants.length} people Ã‚Â· ${record.values.items.length} items`} />
